use crate::geometry::{Annotation, LabelClass, Point, Shape};
...
impl Database {
    ...
    fn init(&self) -> Result<()> {
        ...
        if self.list_classes()?.is_empty() {
            self.add_class("object", [255, 99, 71])?;
            self.add_class("background", [64, 159, 255])?;
        }

        Ok(())
    }

    pub fn upsert_image(&self, hash: &str, width: usize, height: usize, bit_depth: u8, path: &str) -> Result<()> {
        self.conn.execute(
            "
    ...
    pub fn list_classes(&self) -> Result<Vec<LabelClass>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, color_r, color_g, color_b FROM classes ORDER BY name ASC",
        )?;
        let rows = stmt.query_map([], |row| {
            Ok(LabelClass {
                id: row.get(0)?,
                name: row.get(1)?,
                color_rgb: [row.get(2)?, row.get(3)?, row.get(4)?],
            })
        })?;
    ...
    pub fn add_class(&self, name: &str, color_rgb: [u8; 3]) -> Result<i64> {
        self.conn.execute(
            "INSERT INTO classes(name, color_r, color_g, color_b) VALUES (?1, ?2, ?3, ?4)",
    ...
    pub fn list_annotations(&self, image_hash: &str) -> Result<Vec<Annotation>> {
        ...
        let base_rows = stmt.query_map(params![image_hash], |row| {
            Ok((
                ...
                row.get::<_, i64>(1)?,
                row.get::<_, String>(2)?,
                [row.get::<_, u8>(3)?, row.get::<_, u8>(4)?, row.get::<_, u8>(5)?],
                row.get::<_, String>(6)?,
        ...
        match shape {
            Shape::Rectangle { min, max } => {
                self.conn.execute(
                    ...
                    ",
                    params![image_hash, class_id, min.x, min.y, max.x - min.x, max.y - min.y],
                )?;
            }
